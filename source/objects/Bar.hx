...
class Bar extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		...
		bg = new FlxSprite().loadGraphic(Paths.image(image + '/outline'));
		bg.antialiasing = ClientPrefs.data.antialiasing;
		barWidth = Std.int(bg.width - 6);
		barHeight = Std.int(bg.height - 6);

		leftBar = new FlxSprite().loadGraphic(Paths.image(image + '/background'));
		leftBar.antialiasing = antialiasing = ClientPrefs.data.antialiasing;

		rightBar = new FlxSprite().loadGraphic(Paths.image(image + '/background'));
		rightBar.antialiasing = antialiasing = ClientPrefs.data.antialiasing;
