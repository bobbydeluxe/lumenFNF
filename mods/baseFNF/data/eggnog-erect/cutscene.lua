...
function onEndSong()
    if cutsceneFinished == false then
        createInstance('colorShaderCutscene', 'shaders.AdjustColorScreenspace')
        runHaxeCode([[
            getVar('colorShader').setAdjustColor(-17, -13, 7, 4);
            getVar('colorShader').threshold = 1;
        ]])
        setProperty('parentsCutscene.shader', instanceArg('colorShaderCutscene'), nil, true)
        setProperty('santaCutscene.shader', instanceArg('colorShaderCutscene'), nil, true)
